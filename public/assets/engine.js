...
function renderRack() {
  ...
  for (const k of status.keys) {
    ...
    const meta = k.status === "cooldown" ? '<span class="cd">' + t("adm.st.cooldown") + " " + k.cooldown_s + "s</span>" : s.off ? "<span>" + s.text + "</span>" : "<span>ok</span>";
    d.innerHTML = (isNext ? '<div class="next-tag mono">NEXT</div>' : "") + '<div class="prov"><i class="kdot ' + s.dot + '"></i>' + k.label + '</div><div class="kk">' + (k.model ? k.model : "\u2014") + '</div><div class="meta mono">' + meta + "<span>" + t("eng.model") + "</span></div>";
    box.appendChild(d);
  }
}
