...
var msgsEl = byId("msgs");
var colEl = byId("msgCol");
...
function failoverLine(from, code) {
...
function renderSpark() {
  ...
  byId("ttftVal").textContent = avgT > 0 ? (avgT / 1e3).toFixed(2) + "s" : "\u2014";
...
async function send(text) {
  const input2 = byId("input");
  const body = (text ?? input2.value).trim();
  ...
  const dur = firstAt > 0 ? (Date.now() - firstAt) / 1e3 : 0;
  if (meta?.failover) {
