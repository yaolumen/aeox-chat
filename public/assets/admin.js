...
function statusCell(k) {
  ...
  if (k.status === "cooldown") {
    const s = Math.max(0, Math.ceil((k.cooldown_until - Date.now()) / 1e3));
...
function renderTable() {
  ...
  for (const k of keys) {
    ...
    const budgetCell = k.monthly_budget_usd > 0 ? '<span class="bar"><i style="width:' + Math.min(100, k.usage.cost_usd / k.monthly_budget_usd * 100) + '%"></i></span><span class="budg">$' + k.usage.cost_usd.toFixed(2) + "/$" + k.monthly_budget_usd + "</span>" : '<span class="budg">\u2014</span>';
    tr.innerHTML = "<td><b>" + k.provider + '</b></td><td class="kkey">' + (k.key_hint ? "\u2026 " + k.key_hint : "\u2014") + '</td><td class="kkey">' + (k.model || "\u2014") + '</td><td class="mono">P' + k.priority + "</td><td>" + statusCell(k) + '</td><td class="mono">' + k.usage.requests.toLocaleString() + '</td><td class="hide-sm mono">' + fmtTok(k.usage.tokens_in + k.usage.tokens_out) + '</td><td class="hide-sm mono">$' + k.usage.cost_usd.toFixed(2) + "</td><td>" + budgetCell + '</td><td class="acts"></td>';
...
function eventText(e) {
  ...
  if (e.type === "route") {
    return label + " \xB7 " + (typeof d.model === "string" ? d.model : "") + " \xB7 " + String(d.ttft_ms ?? "?") + "ms";
  }
  if (e.type === "cooldown") return label + " " + t("adm.st.cooldown") + " " + String(d.ms ?? 6e4) + "ms \xB7 " + String(d.code ?? "");
  if (e.type === "disabled") return label + " " + t("adm.st.disabled") + " \xB7 " + String(d.code ?? "");
...
async function testKey(id, btn) {
  ...
  try {
    ...
    if (data.ok) {
      btn.textContent = data.latency_ms + "ms \xB7 " + String(data.models ?? "?") + " models";
    } else {
...
function fillPreset() {
  const sel = byId("fProvider");
  const preset = PRESETS[sel.value];
  byId("fBaseUrl").value = preset ? preset.base_url : "";
  byId("fModel").value = preset ? preset.model : "";
  const nameRow = byId("nameRow");
  nameRow.style.display = sel.value === "custom" ? "block" : "none";
}
async function addKey() {
  const sel = byId("fProvider");
  const provider = sel.value === "custom" ? byId("fName").value.trim() : sel.value;
  const base_url = byId("fBaseUrl").value.trim();
  const model = byId("fModel").value.trim();
  const key = byId("fKey").value.trim();
  const priority = Number(byId("fPrio").value) || 100;
  const budget = Number(byId("fBudget").value) || 0;
...
async function loadSeo() {
  try {
    ...
    if (!res.ok) return;
    const data = await res.json();
    byId("seoRobots").value = data.files["robots.txt"] ?? "";
    byId("seoSitemap").value = data.files["sitemap.xml"] ?? "";
    byId("seoLlms").value = data.files["llms.txt"] ?? "";
    byId("seoAi").value = data.files["ai.txt"] ?? "";
    byId("metaTitleEn").value = data.meta.title_en ?? "";
    byId("metaTitleZh").value = data.meta.title_zh ?? "";
    byId("metaDescEn").value = data.meta.desc_en ?? "";
    byId("metaDescZh").value = data.meta.desc_zh ?? "";
    byId("kbText").value = data.system_prompt ?? "";
  } catch {
  }
}
async function saveSeo() {
  const res = await fetch("/admin/api/seo", {
    ...
    headers: { "content-type": "application/json" },
    body: JSON.stringify({
      robots: byId("seoRobots").value,
      sitemap: byId("seoSitemap").value,
      llms: byId("seoLlms").value,
      ai: byId("seoAi").value,
      meta: {
        title_en: byId("metaTitleEn").value,
        title_zh: byId("metaTitleZh").value,
        desc_en: byId("metaDescEn").value,
        desc_zh: byId("metaDescZh").value
      }
    })
  });
  if (res.ok) {
...
async function saveKb() {
  const res = await fetch("/admin/api/seo", {
    method: "PUT",
    headers: { "content-type": "application/json" },
    body: JSON.stringify({ system_prompt: byId("kbText").value })
  });
  if (res.ok) {
...
var providerSel = byId("fProvider");
